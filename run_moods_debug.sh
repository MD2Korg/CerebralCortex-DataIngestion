...
#spark-submit --master $SPARK_MASTER $VIRTUAL_ENV --conf spark.ui.port=$SPARK_UI_PORT --total-executor-cores 50 --driver-memory 10g --executor-memory 1g --py-files $PY_FILES main_debug.py -c $CONFIG_DIRECTORY -bs $BATCH_SIZE -dy $DAY_TO_PROCESS -hr $HOUR_TO_PROCESS -sn $STUDY_NAME
/usr/local/bin/spark-submit --master $SPARK_MASTER $VIRTUAL_ENV --conf spark.ui.port=$SPARK_UI_PORT --total-executor-cores 20 --driver-memory 10g --executor-memory 1g --py-files $PY_FILES "$base_path/main_debug.py" -c $CONFIG_DIRECTORY -bs $BATCH_SIZE -dy $DAY_TO_PROCESS -hr $HOUR_TO_PROCESS -sn $STUDY_NAME
